<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Master Data > Form Tambah Perawat') }}
        </h2>
    </x-slot>
                ...
                <div class="bg-[linear-gradient(to_bottom,#7134FC_0%,#2088FF_100%)] p-6">
                    <div class="flex items-center gap-3">
                        <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                            <i class="fa-solid fa-user-nurse text-white text-2xl"></i>
                        </div>
                        <div>
                            <h3 class="text-xl font-bold text-white">Tambah Data Perawat Baru</h3>
                            <p class="text-blue-100 text-sm">Lengkapi semua informasi Perawat dengan benar</p>
                        </div>
                    </div>
                </div>

                <!-- Form Content -->
                <form action="{{ isset($perawat) ? route('admin.master-perawat.update', $perawat->id) : route('admin.master-perawat.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @if (isset($perawat))
                        @method('PUT')
                    @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="nama" id="nama" value="{{ old('nama', $perawat->user->nama ?? '') }}" required
                            ...
                            <div>
                                ...
                                <input type="text" id="username" name="username" required
                                    value="{{ old('username', $perawat->user->username ?? '') }}"
                                ...
                                <div class="flex gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="jenis_kelamin" id="jenis_kelamin" value="L" required checked
                                            {{ old('jenis_kelamin', $perawat->jenis_kelamin ?? '') == 'L' ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="jenis_kelamin" id="jenis_kelamin" value="P" required
                                            {{ old('jenis_kelamin', $perawat->jenis_kelamin ?? '') == 'P' ? 'checked' : '' }}
                            ...
                            <div>
                                ...
                                <input type="text" id="tempat_lahir" name="tempat_lahir" required
                                    value="{{ old('tempat_lahir', $perawat->tempat_lahir ?? '') }}"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="date" name="tgl_lahir" id="tgl_lahir" required
                                        value="{{ old('tgl_lahir', $perawat->tgl_lahir ?? '') }}"
                                        class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                </div>
                            ...
                            @if (isset($perawat))
                                <div>
                                    <label for="new_password" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                        <input type="hidden" name="old_password" id="old_password" value="{{$perawat->user->password ?? ''}}">
                    ...
                    <div class="border-l-4 border-purple-500 pl-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="no_str" id="no_str" value="{{ old('no_str', $perawat->no_str ?? '') }}" required
                            ...
                            <div>
                                <label for="no_sip" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    No. SIPP (Surat Izin Praktik Perawat) <span class="text-red-500">*</span>
                                </label>
                                <input type="text" name="no_sip" id="no_sip" value="{{ old('no_sip', $perawat->no_sip ?? '') }}" required
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                            ...
                            <!-- No. NIRA -->
                            <div>
                                <label for="no_nira" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    No. NIRA (Nomor Induk Registrasi Anggota) <span class="text-red-500">*</span>
                                </label>
                                <input type="text" name="no_nira" id="no_nira" value="{{ old('no_nira', $perawat->no_nira ?? '') }}" required
                                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                                       placeholder="NIRA-12345">
                                <span class="text-red-600 text-sm">
                                    @error('no_nira'){{ $message }}@enderror
                                </span>
                            </div>

                            <!-- Unit/Poli -->
                            <div>
                                <label for="poli_id" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    Unit / Poli Bertugas <span class="text-red-500">*</span>
                                </label>
                                <select name="poli_id" id="poli_id" required
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200">
                                    <option value="" hidden selected>Pilih Unit/Poli</option>
                                    @foreach ($poli as $p)
                                        <option value="{{ $p->id }}" {{ old('poli_id', $perawat->poli_id ?? '') == $p->id ? 'selected' : '' }}>
                                            {{ $p->nama }}
                                        </option>
                                    @endforeach
                                <span class="text-red-600 text-sm" id="poli_id_error">
                                    @error('poli_id'){{ $message }}@enderror
                                </span>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="alamat" id="alamat" required rows="3"
                                    ...
                                    placeholder="Contoh: Jl. Airlangga No. 45, RT 003 RW 005, Kode Pos 60286">{{ old('alamat', $perawat->alamat ?? '') }}</textarea>
                    ...
                    <div class="flex flex-col sm:flex-row items-center sm:justify-between pt-6 border-t border-gray-200 gap-3 sm:gap-0">
                        ...
                        <!-- Tombol Kembali -->
                        <a href="{{ route('admin.master-perawat.index') }}"
