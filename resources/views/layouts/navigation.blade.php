    ...
    <div class="flex flex-col h-full">
        ...
        {{ request()->routeIs('dashboard') ? 'bg-white/20' : '' }}">
            ...
            @if (Auth::user()->role === 'admin')
                @if (Auth::user()->admin->is_super_admin == 1)
                    ...
                    <div x-data="{ open: false }">
                        ...
                        <!-- Isi Dropdown -->
                        <div x-show="open" x-transition class="ml-8 mt-2 pl-4 border-l border-white/20 space-y-2">
                            <a href="{{ route('superadmin.master-admin.index') }}"
                                class="block text-white hover:text-gray-200">Admin</a>
                        </div>
                    </div>
                @else
                    <!-- Master Data Dropdown -->
                    <div x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition class="ml-8 mt-2 pl-4 border-l border-white/20 space-y-2">
                            <a href="{{ route('admin.master-dokter.index') }}"
                                class="block text-white hover:text-gray-200">Dokter</a>
                            <a href="{{ route('admin.master-perawat.index') }}"
                                class="block text-white hover:text-gray-200">Perawat</a>
                            <a href="{{ route('admin.master-petugas.index') }}"
                                class="block text-white hover:text-gray-200">Petugas</a>
                            <a href="{{ route('admin.master-pasien.index') }}"
                                class="block text-white hover:text-gray-200">Pasien</a>
                            <a href="{{ route('admin.master-poli.index') }}"
                                class="block text-white hover:text-gray-200">Poli</a>
                            <a href="{{ route('admin.master-obat.index') }}"
                                class="block text-white hover:text-gray-200">Obat</a>
                            <a href="{{ route('admin.master-tindakan.index') }}"
                                class="block text-white hover:text-gray-200">Tindakan</a>
                            <a href="{{ route('admin.master-jadwal.index') }}"
                                class="block text-white hover:text-gray-200">Jadwal Tenaga Klinik</a>
                        </div>
                    </div>
                @endif
            @endif
            ...
            <div x-data="{ open: false }" class="mt-2">
                ...
                <!-- Isi Dropdown -->
                <div x-show="open" x-transition class="ml-8 mt-2 pl-4 border-l border-white/20 space-y-2">
                    <a href="{{ route('laporan.kunjungan') }}" class="block text-white hover:text-gray-200">Kunjungan
                        Pasien</a>
                    <a href="{{ route('laporan.pendapatan') }}" class="block text-white hover:text-gray-200">Pendapatan
                        Harian</a>
                    <a href="{{ route('laporan.stokobat') }}" class="block text-white hover:text-gray-200">Stok
                        Obat</a>
                </div>
            </div>
        </nav>
        ...
        <div class="border-t border-white/10 relative">
            ...
            <div x-show="profileOpen" @click.away="profileOpen = false"
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 transition border-t border-gray-200 dark:border-gray-600">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<!-- Overlay for mobile -->
<div x-show="sidebarOpen" @click="sidebarOpen = false" class="lg:hidden fixed inset-0 bg-black/50 z-40"
...
<div class="lg:hidden bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-30">
    <div class="flex items-center justify-between px-4 py-3">
        <button @click="sidebarOpen = true"
            class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 p-2">
            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
